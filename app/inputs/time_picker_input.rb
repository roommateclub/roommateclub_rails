class TimePickerInput < SimpleForm::Inputs::StringInput
  def input
    ...
    template.content_tag :div, class: 'input-group timepicker', data: { 'date-format' => picker_pettern }, id: 'timepicker1' do
      input = super # leave StringInput do the real rendering
      input += template.content_tag :span, class: 'input-group-addon' do
        template.content_tag :i, '', class: 'fa fa-clock-o fa-fixed-width', data: { 'time-icon' => 'fa fa-clock-o icon-time' }
      end
    end
  end
end
